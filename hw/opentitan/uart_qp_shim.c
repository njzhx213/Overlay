                             ...
                             unsigned size)
{
    ...
    if (addr == 0x1C) {
        ...
        if (s->core.uart_core_sys_loopback) {
            if (!fifo8_is_full(&s->rx_buf))
                fifo8_push(&s->rx_buf, byte);
            s->core.uart_core_u_uart_rxfifo_gen_normal_fifo_u_fifo_cnt_wptr_wrap_cnt_q = fifo8_num_used(&s->rx_buf) & 0x7Fu;
            s->core.uart_core_u_uart_rxfifo_gen_normal_fifo_u_fifo_cnt_rptr_wrap_cnt_q = 0;
            ot_uart_qp_update_irqs(s);
        } else {
        ...
        s->core.uart_core_u_uart_txfifo_gen_normal_fifo_u_fifo_cnt_rptr_wrap_cnt_q = s->core.uart_core_u_uart_txfifo_gen_normal_fifo_u_fifo_cnt_wptr_wrap_cnt_q;   /* TX FIFO drained: byte already sent */
    }
