/*
 * QEMU OpenTitan CSRNG device — qemu-passes shim (auto-generated)
 *
 ...
 *
 * Frontend wrapper around the auto-generated CSRNG model from
 *   hw/opentitan/qemu_passes/csrng.{c,h}
 *
 * MMIO read/write callbacks delegate to the generated entrypoints
 * csrng_read() / csrng_write().  The QEMU backend
...
/* Generated csrng model entrypoints + state struct. */
#include "qemu_passes/csrng.h"

/* This shim's own type declaration. */
#include "hw/opentitan/csrng_qp_shim.h"

#define OT_CSRNG_QP_IRQ_NUM 4u

struct OtCsrngQpState {
    SysBusDevice parent_obj;
    MemoryRegion mmio;
    IbexIRQ irqs[OT_CSRNG_QP_IRQ_NUM];
    IbexIRQ alert;

    /* Property fields (mirror upstream ot_csrng_eg.c so SoC-table-injected
...
OBJECT_DEFINE_SIMPLE_TYPE(OtCsrngQpState, ot_csrng_qp, OT_CSRNG_QP, SYS_BUS_DEVICE)

/* === Interrupt delivery ========================================= */
static void ot_csrng_qp_update_irqs(OtCsrngQpState *s)
{
    ...
    csrng_settle(&s->core);
    uint32_t intr_state  = (uint32_t)csrng_read(&s->core, 0x0u, 4);
    uint32_t intr_enable = (uint32_t)csrng_read(&s->core, 0x4u, 4);
    uint32_t masked = intr_state & intr_enable;
    for (unsigned i = 0; i < OT_CSRNG_QP_IRQ_NUM; i++) {
...
static uint64_t ot_csrng_qp_read(void *opaque, hwaddr addr, unsigned size)
{
    ...
    return csrng_read(&s->core, addr, size);
}

static void ot_csrng_qp_write(void *opaque, hwaddr addr, uint64_t value,
                              unsigned size)
{
    OtCsrngQpState *s = OT_CSRNG_QP(opaque);
    csrng_write(&s->core, addr, value, size);
    /* Alert line: a write to ALERT_TEST (IR-derived offset) pulses
     * the device alert — mirrors upstream ot_csrng R_ALERT_TEST. */
    if (addr == 0xCu) {
        ibex_irq_set(&s->alert, (int)(value & 1u));
    }
    ot_csrng_qp_update_irqs(s);
}

static const MemoryRegionOps ot_csrng_qp_ops = {
    .read = ot_csrng_qp_read,
    .write = ot_csrng_qp_write,
    .endianness = DEVICE_LITTLE_ENDIAN,
    .impl.min_access_size = 1,
    .impl.max_access_size = 4,
};

static const Property ot_csrng_qp_properties[] = {
    DEFINE_PROP_STRING(OT_COMMON_DEV_ID, OtCsrngQpState, ot_id),
};

static void ot_csrng_qp_realize(DeviceState *dev, Error **errp)
{
...
static void ot_csrng_qp_init(Object *obj)
{
    OtCsrngQpState *s = OT_CSRNG_QP(obj);
    ...
    memory_region_init_io(&s->mmio, obj, &ot_csrng_qp_ops, s,
                          TYPE_OT_CSRNG_QP, 0x1000);
...
static void ot_csrng_qp_finalize(Object *obj)
{
    (void)obj;
}

static void ot_csrng_qp_class_init(ObjectClass *klass, const void *data)
{
    DeviceClass *dc = DEVICE_CLASS(klass);
    (void)data;

    dc->realize = ot_csrng_qp_realize;
    device_class_set_props(dc, ot_csrng_qp_properties);
